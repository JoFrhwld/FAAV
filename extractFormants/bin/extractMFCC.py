...
class VowelMeasurement:
    ...
    trans = ''          ## word transcription
    preSeg = ''         ## Preceding Segment
    folSeg = ''         ## Following Segment
...
def checkConfigValue(f, option, value):
    ...
    if option == 'outputFormat':
        allowedValues = ['txt', 'text', 'plotnik', 'Plotnik','mfcc']
        checkAllowedValues(f, option, value, allowedValues)
...
def downSample(vowelWavFile, rate):
    os.system('sox ' + os.path.join(SCRIPTS_HOME, vowelWavFile) + ' -r ' + str(rate) + ' ' +  os.path.join(SCRIPTS_HOME, "foo.wav") + ' rate')
    os.remove(os.path.join(SCRIPTS_HOME, vowelWavFile))
    os.rename(os.path.join(SCRIPTS_HOME, "foo.wav"),os.path.join(SCRIPTS_HOME, vowelWavFile))
...
def getVowelMFCC(vowelFileStem, p, w, speechSoftware, measurementPointMethod, padBeg, padEnd, speaker):
    """Gets mfccs"""
    ...
    os.system('praat ' + os.path.join(SCRIPTS_HOME, 'extractMFCC.praat') + ' ' + vowelWavFile + ' ' + str(windowSize) )
    mfcc = praat.MFCC()
    mfcc.read(os.path.join(SCRIPTS_HOME, vowelFileStem + '.MFCC'))
    os.remove(os.path.join(SCRIPTS_HOME, vowelFileStem + '.MFCC'))
    os.remove(os.path.join(SCRIPTS_HOME, vowelWavFile))    

    convertedTimes = []
    coefs = []

    convertedTimes.append(convertTimes(mfcc.times(), p.xmin - padBeg))
    coefs.append(mfcc.mfccs())
    vm = measureVowelMFCC("mfccs",p, w, [], [], coefs, convertedTimes, [], "third", "default", padBeg, padEnd, '', '')
    return vm
...
def getVowelMeasurement(vowelFileStem, p, w, speechSoftware, formantPredictionMethod, measurementPointMethod, nFormants, maxFormant, windowSize, preEmphasis, padBeg, padEnd, speaker):
    ...
    ## default:
        ...
        vm = measureVowel("formants", p, w, formants, bandwidths, convertedTimes, intensity, measurementPointMethod, formantPredictionMethod, padBeg, padEnd, '', '')
...
def measureVowelMFCC(measureType, phone, word, poles, bandwidths, coefs, times, intensity, measurementPointMethod, formantPredictionMethod, padBeg, padEnd, means, covs):
    measurementPoint = getMeasurementPoint(phone, [], times[0], intensity, measurementPointMethod)
    ...
    mfccs_selected = coefs[0][i-1:i+2]

    f1, f2, f3, b1, b2, b3 = [1, 1, 1, 1, 1, 1]
    ...
    vm.mfccs = mfccs_selected                   ## selected mfcss
    vm.nFormants = 0
    return vm


def measureVowel(measureType, phone, word, poles, bandwidths, mfccs, times, intensity, measurementPointMethod, formantPredictionMethod, padBeg, padEnd, means, covs):
...
def outputMeasurements(outputFormat, measurements, speaker, outputFile, outputHeader):
    ...
    elif outputFormat in ['mfccs', "mfcc"]:
        fw = open(outputFile, 'w')
        header = ["vowel", "stress", "word", "t", "beg", "end","dur", "cd", "fm",'fp', 'fv', 'ps', 'fs', 'style', 'glide']
        for i in range(3):
            for j in range(13):
                header.append("m" + str(i+1) + "_" + str(j))
        fw.write('\t'.join(header))
        fw.write("\n")
        for vm in measurements:
            fw.write('\t'.join([vm.phone, str(vm.stress), vm.word]))
            fw.write('\t')
            fw.write('\t'.join([str(vm.t), str(vm.beg), str(vm.end), str(vm.dur), vm.cd, vm.fm, vm.fp, vm.fv, vm.ps, vm.fs, vm.style, vm.glide]))
            fw.write('\t')

            for mfcc_set in vm.mfccs:
                for coef in mfcc_set:
                    fw.write(repr(coef))
                    fw.write("\t")
                nCoef = len(mfcc_set)
                while nCoef < 13:
    ...
    for (wavFile, tgFile, outputFile) in zip(wavFiles, tgFiles, outputFiles):
        ...
        for w in words:
            ...
            for p in w.phones:
                ...
                ## rate = 11000
                ## downSample(vowelWavFile, rate)

                
                vm = getVowelMFCC(vowelFileStem, p, w, speechSoftware, measurementPointMethod, padBeg, padEnd, speaker)
                print "Got measurement for %s" %(vm.phone)
        ...
        if len(measurements) > 0:
            outputMeasurements("mfcc", measurements, speaker, outputFile, outputHeader)
